...
st.set_page_config(page_title="CareerCanvas AI", page_icon="CC", layout="wide")
...
<style>
    ...
    :root { --ink: #413B46; --primary: #887C9E; --secondary: #B3A8C3; --paper: #F5F1EA; --card: #FCFAF7; --accent: #D8CFDF; --line: rgba(65,59,70,.16); --muted: #756C78; }
    ...
    .eyebrow { color: #887C9E; font-weight: 700; letter-spacing: .16em; text-transform: uppercase; font-size: .78rem; }
    .hero-copy { color: #756C78; font-size: 1.08rem; max-width: 650px; line-height: 1.6; }
    .section-kicker { color: #887C9E; font-size: .72rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; margin: 1.2rem 0 .35rem; }
    .module-card { background: #FCFAF7; color: #413B46; border: 1px solid #D8CFDF; border-radius: 1.2rem; padding: 1rem; min-height: 118px; box-shadow: 0 14px 32px rgba(65,59,70,.08); }
    .module-card strong { display: block; font-family: 'Space Grotesk', sans-serif; font-size: 1.02rem; margin-bottom: .35rem; }
    .module-card span { color: #756C78; font-size: .86rem; line-height: 1.4; }
    .panel { background: #FCFAF7; border: 1px solid #D8CFDF; padding: 1.25rem; border-radius: 1.2rem; box-shadow: 0 14px 34px rgba(65,59,70,.08); }
    .metric { background: #887C9E; color: #FCFAF7; padding: 1rem 1.1rem; border-radius: 1rem; box-shadow: 0 10px 22px rgba(136,124,158,.16); }
    .metric span { color: #F5F1EA; font-size: .78rem; }
    .metric strong { font-size: 1.8rem; display: block; }
    .stButton > button, .stDownloadButton > button { border-radius: 999px; font-weight: 700; min-height: 2.7rem; border-color: #887C9E; color: #FCFAF7; background: #887C9E; }
    [data-testid="stSidebar"] .stButton > button { text-align: left; padding-left: .9rem; }
    [data-testid="stForm"] { background: #FCFAF7; border: 1px solid #D8CFDF; border-radius: 1rem; padding: 1.2rem; }
    [data-testid="stExpander"] { border-color: #D8CFDF; background: #FCFAF7; }
...
st.markdown('<div class="hero"><div class="eyebrow">Generative career studio</div><h1>CareerCanvas AI</h1><p class="hero-copy">One focused path from first draft to interview-ready confidence.</p></div>', unsafe_allow_html=True)
...
theme_css = """
:root { --ink: #413B46; --muted: #756C78; --surface: #FCFAF7; --surface-2: #FCFAF7; --line: #D8CFDF; --accent: #887C9E; --primary: #887C9E; }
.stApp { background: #F5F1EA; }
"""
...
<style>
    ...
    [data-testid="stSidebar"] {{ background: #FCFAF7; }}
    ...
    [data-testid="stSidebar"] .stButton > button {{ color: #413B46 !important; background: #B3A8C3 !important; border-color: #D8CFDF !important; }}
    [data-testid="stSidebar"] .stButton > button[kind="primary"] {{ color: #FCFAF7 !important; background: #887C9E !important; border-color: #887C9E !important; }}
    .panel {{ background: #FCFAF7; border-color: #D8CFDF; }}
    .module-card, [data-testid="stForm"], [data-testid="stExpander"] {{ background: #FCFAF7; border-color: #D8CFDF; }}
...
if mode == "Build resume":
    ...
    st.write("Give CareerCanvas AI your real details. It will structure and polish them without inventing experience.")
